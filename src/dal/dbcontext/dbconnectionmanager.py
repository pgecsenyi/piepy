...
class DbConnectionManager(object):
    ...
    def acquire_cursor(self, check_path=True):

        return self._connect(check_path).cursor
    ...
    def _close(self, do_commit):

        thread_id = threading.current_thread().ident
        if thread_id in self._connections:
            if self._connections[thread_id].close(do_commit):
                del self._connections[thread_id]

    def _connect(self, check_path=True):
